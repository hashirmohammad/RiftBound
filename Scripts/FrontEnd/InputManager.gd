...
func _try_start_legend_targeting(clicked_card: CardInstance) -> void:
	var state: GameState = game_controller.state
	var active_player: PlayerState = state.get_active_player()

	if active_player.legend == null or active_player.legend.uid != clicked_card.uid:
	...
	_highlight_friendly_units_for_legend()
...
func _highlight_friendly_units_for_legend() -> void:
	_clear_slot_highlights()

	var active_id: int = game_controller.state.get_active_player().id

	var board_slots: Array = board_reference._player_slot_nodes if active_id == 0 else board_reference._p1_slot_nodes
...
func _get_rune_instance(rune_uid: int) -> RuneInstance:
	var state: GameState = game_controller.state
	var player: PlayerState = state.players[state.pending_payment_player_id] if state.awaiting_rune_payment and state.pending_payment_player_id != -1 else state.get_active_player()
